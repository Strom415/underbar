(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function (noFlag, ele) {
      var pass = ele;
      if (hasIterator) {
        pass = iterator(ele);
      }
  ...
  _.some = function(collection, iterator) {
    ...
    return _.reduce(collection, function(acc, ele) {
      var passTest = _.identity(ele);

      if (hasIterator) {
        var passTest = iterator(ele);
      }
  ...
  _.shuffle = function(array) {
    ...
    for (var i = array.length; i > 1; i--) {
      var nextUp = random(i);
      storage.push(clone[nextUp]);
      clone.splice(nextUp, 1);
    }

    storage.push(clone[0]);
    return storage;
  };
